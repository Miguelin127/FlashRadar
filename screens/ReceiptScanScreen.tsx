...
export const ReceiptScanScreen = () => {
  ...
  if (!isPremium) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: darkMode ? '#000' : '#fff' }}>
        <Text style={{ fontSize: 18, fontWeight: 'bold', color: darkMode ? '#fff' : '#000', marginBottom: 10 }}>
          Premium Feature
        </Text>
        <Text style={{ color: darkMode ? '#ccc' : '#666' }}>Upgrade to unlock receipt scanning</Text>
      </View>
    );
  }
  ...
  const scanReceipt = async (base64: string, mimeType: string) => {
    setLoading(true);
    try {
      const scanFn = httpsCallable(functions, 'scanReceipt');
  ...
  if (!image) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: bgColor, gap: 15 }}>
        <Text style={{ fontSize: 20, fontWeight: 'bold', color: textColor, marginBottom: 20 }}>Scan Receipt</Text>
        <TouchableOpacity
          onPress={takePhoto}
          style={{
  ...
  if (loading) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: bgColor }}>
        ...
        <Text style={{ color: textColor, marginTop: 15 }}>Scanning receipt...</Text>
      </View>
    );
  }
  ...
  if (receipt) {
    return (
      <ScrollView style={{ flex: 1, backgroundColor: bgColor, padding: 15 }}>
        <Text style={{ fontSize: 18, fontWeight: 'bold', color: textColor, marginBottom: 15 }}>Review Items</Text>
        <Text style={{ color: textColor, marginBottom: 5 }}><strong>Store:</strong> {receipt.store}</Text>
        <Text style={{ color: textColor, marginBottom: 15 }}><strong>Date:</strong> {receipt.date}</Text>
        ...
        {receipt.items.map((item: any, i: number) => (
          <TouchableOpacity
            ...
            style={{
              ...
              borderColor: selectedItems.includes(i) ? '#FF7A00' : borderColor,
        ...
        <Text style={{ color: textColor, marginTop: 15, fontSize: 16, fontWeight: 'bold' }}>
        ...
        >
          <Text style={{ color: textColor, textAlign: 'center', fontWeight: 'bold' }}>← Back</Text>
        </TouchableOpacity>
      </ScrollView>
    );
  }
