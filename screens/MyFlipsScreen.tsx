...
export default function MyFlipsScreen() {
  ...
  const exportCSV = async () => {
    ...
    const rows = flips.map((f) => {
      ...
      return `"${(f.title || "Manual Flip").replace(/"/g, '""')}",${invested},${sell},${profit.toFixed(2)},${roi},"${f.source || ""}",${ts}`;
    }).join("\n");
  ...
  const exportPDF = async () => {
    ...
    const html = `<html><body style="font-family:-apple-system;padding:24px;">
      ...
      <p><strong>Today:</strong> $${stats.today.toFixed(2)}<br/>
      <strong>Week:</strong> $${stats.week.toFixed(2)}<br/>
      <strong>All Time:</strong> $${stats.total.toFixed(2)}</p>
      <table width="100%" border="1" cellspacing="0" cellpadding="8">
        <thead><tr><th>Title</th><th>Invested</th><th>Sell</th><th>Profit</th><th>ROI</th></tr></thead>
        <tbody>${rows}</tbody>
      </table></body></html>`;
  ...
  const deleteFlip = async (id: string) => {
    if (!user) return;
    // ── Compat SDK ──────────────────────────────────────────────────────────
    await db.collection("users").doc(user.uid).collection("flips").doc(id).delete();
  };
  ...
  const savePrices = async (flip: Flip) => {
    ...
    // ── Compat SDK ──────────────────────────────────────────────────────────
    await db.collection("users").doc(user.uid).collection("flips").doc(flip.id).update({
      buyPrice: invested,
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: bg }]}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={[styles.summary, { backgroundColor: cardBg }]}>
          <Text style={{ color: textSecondary }}>Today: <Text style={{ color: stats.today >= 0 ? green : red, fontWeight: "900" }}>${stats.today.toFixed(2)}</Text></Text>
          <Text style={{ color: textSecondary }}>Week: <Text style={{ color: stats.week >= 0 ? green : red, fontWeight: "900" }}>${stats.week.toFixed(2)}</Text></Text>
          <Text style={{ color: textSecondary }}>All Time: <Text style={{ color: stats.total >= 0 ? green : red, fontWeight: "900" }}>${stats.total.toFixed(2)}</Text></Text>
        </View>
            ...
            >
              <View style={[styles.card, { backgroundColor: cardBg }]}>
                <View style={styles.row}>
                  <Text style={[styles.cardTitle, { color: textPrimary }]}>{flip.title || "Manual Flip"}</Text>
                ...
                <Text style={{ color: textSecondary }}>Invested ${invested} → Sell ${sell}</Text>
                ...
                {editingId === flip.id ? (
                  <>
                    <TextInput value={buyInput} onChangeText={setBuyInput} keyboardType="numeric" placeholder="Total invested" placeholderTextColor={textSecondary} style={[styles.input, { color: textPrimary, borderColor: accent }]} />
                    <TextInput value={sellInput} onChangeText={setSellInput} keyboardType="numeric" placeholder="Sell price" placeholderTextColor={textSecondary} style={[styles.input, { color: textPrimary, borderColor: accent }]} />
                    <Pressable style={styles.saveBtn} onPress={() => savePrices(flip)}>
                      <Text style={styles.saveText}>Save</Text>
                    </Pressable>
                    <Pressable onPress={() => { setEditingId(null); setBuyInput(""); setSellInput(""); }}>
                      <Text style={[styles.editText, { marginTop: 10 }]}>Cancel</Text>
                    </Pressable>
                  </>
                ) : (
                  <Pressable onPress={() => { setEditingId(flip.id); setBuyInput(String(invested)); setSellInput(flip.sellPrice !== undefined ? String(flip.sellPrice) : ""); }}>
...
const styles = StyleSheet.create({
  ...
  editText: { color: "#FF8C00", fontWeight: "800", marginTop: 6 },
  targetLine: { marginTop: 6, fontWeight: "800" },
  input: { borderWidth: 1, borderRadius: 10, padding: 10, marginTop: 8 },
  saveBtn: { backgroundColor: "#FF8C00", paddingVertical: 10, borderRadius: 10, marginTop: 6, alignItems: "center" },
