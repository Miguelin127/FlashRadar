...
exports.analyzePriceHistory = analyzePriceHistory;
/**
...
function analyzePriceHistory(priceHistory) {
    ...
    const earlyAvg = earlySlice.reduce((sum, p) => sum + p, 0) / earlySlice.length;
    const lateAvg = lateSlice.reduce((sum, p) => sum + p, 0) / lateSlice.length;
    let trend = "FLAT";
