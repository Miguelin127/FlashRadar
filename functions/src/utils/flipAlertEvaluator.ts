// FlashRadarProject/utils/flipAlertEvaluator.ts

import { analyzeFlip } from "../services/analyzeFlip";
import { FlipItem } from "./FlipItem";
import { PricePoint } from "./priceHistory";

interface AlertCheckInput {
  existingFlip: FlipItem;
  updatedPriceHistory: PricePoint[];
  platformInputs: any;
}

interface AlertResult {
  shouldNotify: boolean;
  newFlip: FlipItem;
  reason?: string;
}

export function evaluateFlipAlert(
  input: AlertCheckInput
): AlertResult {
  const { existingFlip, updatedPriceHistory, platformInputs } = input;

  // Only monitor WAIT flips
  if (existingFlip.verdict !== "WAIT") {
    return {
      shouldNotify: false,
      newFlip: existingFlip,
    };
  }

  const reAnalyzedFlip = analyzeFlip({
    userId: existingFlip.userId,
    title: existingFlip.title,
    buyPrice: existingFlip.buyPrice,
    priceHistory: updatedPriceHistory,
    platformInputs,
    demand: existingFlip.demand,
    dealOrigin: existingFlip.dealOrigin,
    source: existingFlip.source,
  });

  // WAIT → BUY transition
  if (reAnalyzedFlip.verdict === "BUY") {
    return {
      shouldNotify: true,
      newFlip: reAnalyzedFlip,
      reason: "Price hit optimal buy zone",
    };
  }

  return {
    shouldNotify: false,
    newFlip: reAnalyzedFlip,
  };
}
